...
//신고가 가능한 시간대
if(  (date("H") >= 0 && date("H") <= 11) )  $enable_siren=true;
...
?>
						...
						<!--li>
						<p>입금은행</p>
						<p class="btn-clipboard" data-clipboard-text="<?=$row[tr_bank]?> <?=$row[tr_bank_num]?> <?=$row[tr_bank_user]?>"   ><?=$row[tr_bank]?$row[tr_bank]:'-'?> <?=$row[tr_bank_num]?> <?=$row[tr_bank_user]?> <i class="far fa-copy"></i></p>

						</li--> 
						
						<!--li>
						<p>USDT Address</p>
						<p class="btn-clipboard text-narrow0" data-clipboard-text="<?=$row[tr_wallet_addr]?>"  ><?=$row[tr_wallet_addr]?$row[tr_wallet_addr]:'-'?> <i class="far fa-copy"></i></p>
                   ...
                   <div class='mt-3'>
					<input name="tr_txid" type="text" class="my-1" id="tr_txid" value="<?=$row[tr_txid]?>" placeholder="Txid Hash <?=lng('복사 후 붙여넣기')?>">
					
                    <ul class="notice">
                        <li><?=lng('테더 구매시 txid를 입력치 않으면 구매완료 처리 되지 않습니다.')?></li>
                    </ul>
                    <div  class="Btn  btn-deposit w-80 mx-auto" data-code='<?=$row[tr_code]?>' ><a href="#"><?=lng('결제확인')?></a></div>
					</div>
					
						
					<div class='mt-3'>
					<p>신고사유</p>					
					 <textarea name='tr_buyer_memo' id='tr_buyer_memo' class='form-control text-limit' text-limit-len='100' rows='4' placeholder="불량거래 신고를 하시려면 사유를 입력하신후 신고 버튼을 클릭하세요." 
					 <?=$enable_siren==false ?'disabled':''?> ><?=$row[tr_buyer_memo]?></textarea>
					 
					 <?						
				if($enable_sire ){?>	
					<p><is class='text-legnth-val'>0</is>/<?=lng('100자 이내')?></p>						
                    <!--div  class="Btn btn-complete w-80 mx-auto  btn-buyer-claim"  <?=$enable_siren==false ?'disabled':''?> data-code='<?=$row[tr_code]?>'  ><a href="#">신고</a></div-->
                    <? }?>
...
?>	
		...
		<?=$row[bmb_hp]?></p>
                        ...
                        <li>
                            <p>가격 : <?=number_format2(floor($row[ct_sell_price]/10)*10)?>원</p>
                        </li>
                        <li>
...
?>
						...
						<div class='mt-3'>
					<p>신고사유</p>					
					 <textarea name='tr_seller_memo' id='tr_seler_memo' class='form-control text-limit' text-limit-len='100' rows='4' placeholder="불량거래 신고를 하시려면 사유를 입력하신후 신고 버튼을 클릭하세요." 
					 <?=$enable_siren==false ?'disabled':''?> ><?=$row[tr_seller_memo]?></textarea>
					 
					 <?						
				if($enable_sire){?>		
					<p><is class='text-legnth-val'>0</is>/<?=lng('100자 이내')?></p>
					
                    <div  class="Btn btn-complete w-80 mx-auto  btn-seller-claim"  <?=$enable_siren==false ?'disabled':''?> data-code='<?=$row[tr_code]?>'  ><a href="#">신고</a></div>
                    <? }?>
					
			 		 </div>						
                    
					<div class='mt-3'>
                    <!--div class="longBtn Btn w_btn"><a href="#">입금자명 + 아이디</a></div-->
					<ul class="dealDesc">	
					<li>	
					<p><?=lng('테더 전송 거래 번호')?></p>
					<p><?=$row[tr_txid]?$row[tr_txid]:'-'?></p>
					</li>
					</ul>
					<button class='Btn smallBtn txid-btn'  data-code="<?=$row[tr_code]?>" ><?=lng('TXID조회')?></button>
							
							
                    <ul class="notice mt-3">						
                        <li>입금이 된것을 확인한후 반드시 입금확인 버튼을 클릭하세요 <li>
                    </ul>                  
                    <div  class="Btn btn-complete w-80 mx-auto" data-code='<?=$row[tr_code]?>' ><a href="#">입금확인</a></div>
					</div>
...
<script>
	...
	function page_reload(){
		window.location.href='./trade.php?stats_stx=<?=$stats_stx?>';
	}
					...
					$.ajax({
						...
						success: function(data) {
							...
							if(data.result==true){					
								...
								Swal.fire({html:'<?=lng('거래를 종료하였습니다')?>',timer:2000,
								onClose: () => {page_reload()}									
							   });
