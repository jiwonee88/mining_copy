...
if ( $w == 'u') {
	...
	if($tr_amt< $tr_fee) alert_json(false,"전송 수수료 보다 큰 금액을 전송해야 합니다");	
	
	//최소금액
	if($cset['min_trans_'.$tr_token] &&  $cset['min_trans_'.$tr_token] > $tr_amt ) alert_json(false,"전송 최소 금액 : $".number_format2($cset['min_trans_'.$tr_token]));
