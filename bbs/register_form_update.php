...
if (!($w == '' || $w == 'u')) {
    alert_json(false,'The wrong approach.');
}
/*
if ($w == 'u' && $is_admin == 'super') {
    if (file_exists(G5_PATH.'/DEMO'))
        alert_json(false,'데모 화면에서는 하실(보실) 수 없는 작업입니다.');
}
*/
...
if($w == 'u')
    ...
    alert_json(false,'잘못된 접근입니다', G5_URL);

if(!$mb_id)
    alert_json(false,'아이디를 입력하세요');

$mb_password    = trim($_POST['mb_password']);
$mb_password_re = trim($_POST['mb_password_re']);
if(trim($_POST['mb_name'])!='' ) $mb_name        = trim($_POST['mb_name']);
else $mb_name        = $mb_id;
$mb_nick        = trim($_POST['mb_name']); //trim($_POST['mb_nick']);
...
$mb_recommend2   = isset($_POST['mb_recommend2'])     ? trim($_POST['mb_recommend2'])   : "";
...
if(is_array($_POST['mb_5'])) $mb_5_val=implode("|",$_POST['mb_5']);
else $mb_5_val='';
...
if ($w == '' || $w == 'u') {
	...
	*/
    ...
    if($w=='' && $tmp_mb_name != $mb_name) {
        alert_json(false,'옳바른 이름을 입력하세요');
    }
    $tmp_mb_nick = iconv('UTF-8', 'UTF-8//IGNORE', $mb_nick);
    if($tmp_mb_nick != $mb_nick) {
        //alert_json(false,'닉네임을 올바르게 입력해 주십시오.');
    }

    if ($w == '' && !$mb_password)
        alert_json(false,'암호를 입력하세요');
    if($w == '' && $mb_password != $mb_password_re)
        alert_json(false,'암호가 일치하지 않습니다');

    //if ($w == '' &&  $msg = empty_mb_name($mb_name))       alert($msg, "", true, true);
   ...
   // if ($msg = valid_mb_nick($mb_nick))     alert($msg, "", true, true);
    if ($mb_email && $msg = valid_mb_email($mb_email))   alert_json(false,$msg, "");
    if ($mb_email && $msg = prohibit_mb_email($mb_email)) alert_json(false,$msg, "");
		...
		*/
        ...
        if ( $mb_recommend) {
            if (!exist_mb_id($mb_recommend))
                alert_json(false,"추천인 코드가 존재하지 않습니다");
        }
        if (strtolower($mb_id) == strtolower($mb_recommend)) {
            alert_json(false,'회원님의 아이디를 추천인 코드로 입력할수 없습니다');
        }	
		
		 if(!preg_match("/^[0-9]{7}$/",$mb_12))  {
            alert_json(false,'UID는 7자리 숫자로 이뤄져야 합니다');
        }	
		
		$temp=sql_fetch("select * from $g5[member_table] where mb_12='$mb_12'");
		if ( $temp[mb_id]) {          
            alert_json(false,"입력하신 UID는 이미 사용중입니다");
        }
    ...
    if ($mb_email && $msg = exist_mb_email($mb_email, $mb_id))   alert_json(false,$msg);
}
...
if ($w == '' || $mb_deposite_pass!='' ) {
	$sql_certify .= " , mb_deposite_pass = '".get_encrypt_string($mb_deposite_pass)."'";
}
...
if ($w == '') {
    ...
    if ($config['cf_email_mb_member']) {
        $subject = '['.$config['cf_title'].'] Congratulations!';
    ...
    if (!trim($_SESSION['ss_mb_id']))
        alert_json(false,'You are not logged in.');

    if (trim($_POST['mb_id']) != $mb_id)
        alert_json(false,"Your login and the information you're trying to edit are incorrect, so they can't be edited. \\nIf you're using the wrong method, stop right away.");
...
if ($msg) alert_json($msg);

 //   echo '<script>alert(\''.$msg.'\');</script>';

if ($w == '') {
	alert_json(true,'ok', array('goto_url'=> G5_URL) );
    ...
    if ($old_email != $mb_email && $config['cf_use_email_certify']) {
        ...
        alert_json(true,"Member information has been modified.\nYour e-mail address has changed and you will need to reauthenticate.", array('goto_url'=> G5_URL) );
    } else {
		alert_json(true,'ok');
        echo '
        <!doctype html>
