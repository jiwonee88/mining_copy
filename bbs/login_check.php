...
if (!$mb_id || !$mb_password)
    alert_json(false,'Member ID or Password must not be blank.');

$mb = get_member($mb_id);
//if(!$mb) $mb = get_emailmember($mb_id);
...
if ($mb['mb_intercept_date'] && $mb['mb_intercept_date'] <= date("Ymd", G5_SERVER_TIME)) {
    ...
    alert_json(false,'Your ID is not allowed. Processed: '.$date);
}
...
if ($mb['mb_leave_date'] && $mb['mb_leave_date'] <= date("Ymd", G5_SERVER_TIME)) {
    ...
    alert_json(false,'탈퇴한 아이디이므로 접근하실 수 없습니다.\nWithdrawal date : '.$date);
}
*/
//탈퇴한 회원
if ($mb['mb_10']=='2' ) {
    if ($mb['mb_leave_date'] && $mb['mb_leave_date'] <= date("Ymd", G5_SERVER_TIME)) $date = ' Withdrawal date : '.preg_replace("/([0-9]{4})([0-9]{2})([0-9]{2})/", "\\1. \\2. \\3.", $mb['mb_leave_date']);
    alert_json(false,"You can't access this account because you've opted out.".$date);
}

//삭제한 회원
if ($mb['mb_10']=='1' ) {
    if ($mb['mb_leave_date'] && $mb['mb_leave_date'] <= date("Ymd", G5_SERVER_TIME)) $date = ' Deletion date : '.preg_replace("/([0-9]{4})([0-9]{2})([0-9]{2})/", "\\1. \\2. \\3.", $mb['mb_leave_date']);
    alert_json(false,"Can't access because it is deleted.".$date);
}

// 메일인증 설정이 되어 있다면
if ( is_use_email_certify() && !preg_match("/[1-9]/", $mb['mb_email_certify'])) {
    $ckey = md5($mb['mb_ip'].$mb['mb_datetime']);
     alert_json(false,"{$mb['mb_email']} You must be able to log in by email verification. Click Cancel to change to a different email address and verify.",array('goto_url'=> G5_BBS_URL.'/register_email.php?mb_id='.$mb['mb_id'].'&ckey='.$ckey));
}
...
if ($remember_id) {
    ...
    // 자동로그인 ---------------------------
    // 쿠키 한달간 저장
    set_cookie('remember_mb_id', $mb['mb_id'], 86400 * 31);
	
} else {
    set_cookie('remember_mb_id', '', 0);
}
...
alert_json(true,'ok',array('goto_url'=>$link));
//goto_url($link);
?>
