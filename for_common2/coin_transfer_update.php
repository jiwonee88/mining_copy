...
if ( $w == 'u') {
	...
	if($stmb_id=='' || $tr_token=='') alert_json(false,'ERROR');	
	
	if($stmb_id=='') alert_json(false,'이체를 받을 아이디를 입력하세요');	
	
	if($member[mb_id]==$stmb_id) alert_json(false,"동일한 아이디에 전송할수 없습니다");
	...
	if( $tr_amt == 0) alert_json(false,'이체할 수량을 입력하세요');
	
	if($sum < abs($tr_amt))  alert_json(false,'이체할 수량이 부족합니다');
		
	//활성화 비용 이하는 출금 불가
	if( $cset[staking_amt] > ($sum - $tr_amt) )  alert_json(false,'이체후 잔액이 활성화 최소 보유량 미만입니다');
		
	if( $tr_amt < $tr_fee) alert_json(false,"이체할 수량이 수수료 보다 커야 합니다.");	
	...
	if($cset['min_trans_'.$tr_token] > $tr_amt ) alert_json(false,"최소 이체 수량보다 적습니다 : ".number_format2($cset['min_trans_'.$tr_token]));
