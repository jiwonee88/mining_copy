...
function service_block($type='json'){
	global $g5,$cset,$member;
	
	//백그라운드 실행시
	if($type=='json'){
		if($cset[service_block] =='1') alert_json(false,"매칭 진행중 또는 시스템 점검중입니다<br>잠시 서비스 이용이 정지중입니다");
		if($member[mb_trade_penalty] > 1 ) alert_json(false,"회원님은 매칭 거래 불량 회원으로 2회 이상 신고되어<br>서비스 이용이 불가능합니다. ");		
	}
...
function set_purchase_item($data,$status,$exec=0){
	...
	if($data['in_stats']!='3' && $status=='3'){
		...
		for($i=1;$i <= $data[it_item_qty];$i++){
			...
			//예정가격
			$sell_price=floor( ($item_data[price] + ($item_data[price]*$item_data[interest]/100))/10 )*10;
			...
			$ct_id=sql_insert_id();
			$code= get_itemcode($data[cn_item],$ct_id);	
	...
	//구매 취소
	if(($data['in_stats']=='3' && $status!='3') || $status=='del'){
		
		//삭제 가능 상품인지 검사
		$cart_code_arr=explode(",",$data['cart_code']);
...
function set_transfer_coin($data,$status,$exec=0){
	...
	//print_r($data).$status.$amt;
	$mb=get_member($data['mb_id']);
	$tmb=get_member($data['tmb_id']);
	if($data['stmb_id']){
	...
	if($data['tr_stats']!='3' && $status=='3'){
		...
		set_add_point('transout',$mb,'','',$content);		
...
function set_trade_stat($tdata,$stats){
	...
	if($stats=='3'){
		...
		if($tdata[cart_code]){
			...
			//단일 상품 판매는 소유권 이전
				...
				$ct_class=$cdata[ct_class]+1;				
				
				//이전 상품 상태 백업
				$cdata[ct_backup]='';
				
				$ct_backup=addslashes(json_encode($cdata));
			...
			$sql = " update {$g5['cn_item_cart']}
						...
						is_soled= if(soled_cnt >= div_cnt and soled_cnt > 0,1,0) ,						
						soled_date=if(is_soled='1',now(),'')

						where code='$tdata[cart_code]'";
			*/
			//판매된 상품 상품삭제
			//if($cdata[div_cnt] == 1 ) $sql = " delete from {$g5['cn_item_cart']} where code='$tdata[cart_code]'";		
			//$result= sql_query($sql);	
			
			if(!$result) alert_json(false,'처리할수 없습니다');	
	...
	if( $stats!='3' && $tdata['tr_stats']=='3'  ){
		...
		//새로 지급된 상품은 삭제
		if($tdata['to_cart_code'] != $tdata['cart_code'] ){
		...
		//소유권 이전된 상품은 다시 되돌리
			...
			$old_cdata=json_decode($cdata[ct_backup]);
			
			$sql="update {$g5['cn_item_cart']} set
				set 
				tr_code='$old_cdata[tr_code]',					
				mb_id='".addslashes($tdata[fmb_id])."',
				smb_id='".addslashes($tdata[fsmb_id])."',
				fmb_id='".addslashes($old_cdata[fmb_id])."',
				fsmb_id='".addslashes($old_cdata[fsmb_id])."',

				ct_buy_price='$old_cdata[tr_price_org]',
				ct_sell_price='$old_cdata[ct_sell_price]',

				ct_class='$old_cdata[ct_class]',
				ct_interest='$old_cdata[interest]',
				ct_days='$old_cdata[days]',
				ct_validdate='$old_cdata[ct_validdate]',

				ct_wdate='$old_cdata[tr_wdate]',

				tr_active='$old_cdata[tr_active]'
	...
	if(($stats=='9' || $stats=='del')  && $tdata['tr_stats']!='9'){
		...
		if($tdata[cart_code] ){
			  ...
			  $sql = "update {$g5['cn_item_cart']}
					...
					is_trade = if(trade_cnt >= 1 and div_cnt > 1,2,if(trade_cnt >= 1,1,0))
		...
		//수수료 회수 - 구매
		set_del_point('mfee',$tdata['mb_id'],'','',substr($tdata['tr_code'],0,20),$tdata[tr_rdate]) ;
		//수수료 회수 - 판매
		set_del_point('mfee2',$tdata['fmb_id'],'','',substr($tdata['tr_code'],0,20),$tdata[tr_rdate]);	
	...
	if($stats!='9' && $tdata['tr_stats']=='9'){
			...
			if($tdata[cart_code] ){
				 ...
				 $trade_cnt=$cdata[trade_cnt]+1;
				 
				 if($trade_cnt >= $cdata[div_cnt]) $is_trade=1;
				 else if($trade_cnt > 0) $is_trade=2;
				 else $is_trade=0;			
			...
			if($tdata[tr_seller_fee] > 0 ){
				...
				set_add_point('mfee2',$data,$tdata[fsmb_id],$member['mb_id'],$content);						
			}
...
function set_update_point($mb_id,$smb_id=''){	
	...
	mb_point_out_i='".$p['out']['w']."',
	mb_point_out_u='".$p['out']['t']."'		
