...
// 회원 삭제
function member_delete($mb_id,$alldel=0)
{
    global $config;
    global $g5;
    ...
    // 이미 삭제된 회원은 제외
    if(preg_match('#^[0-9]{8}.*삭제함#', $mb['mb_memo']) && !$alldel)
        return;
	...
	//조직도 삭제
	del_mb_treedb($mb,'tree');
	del_mb_treedb($mb,'tree2');	
	...
	while($data=sql_fetch_array($result)){			
		sql_query(" delete from {$data['TABLE_NAME']} where mb_id = '$mb_id' ");
	}	
	sql_query(" delete from {$g5['cn_pointsum']} where mb_id = '$mb_id' ");	
	
	//서브 계정 삭제
	sql_query(" delete from {$g5['cn_sub_account']} where mb_id = '$mb_id' ");	
	
	//트리 삭제
	del_mb_treedb($mb['mb_id'],'','treedb'); 
	del_mb_treedb('',$mb['mb_id'],'treedb'); 	
