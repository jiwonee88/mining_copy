...
액면가:
<input name="org_price[<?=$k?>]" type="text"  class="frm_input number-comma" id="org_price_<?=$k?>" size="15" value='<?=number_format2($v[price])?>' />
보유기간:
<input name="org_days[<?=$k?>]" type="text"  class="frm_input" id="org_days_<?=$k?>" size="7" value='<?=$v[days]?>' />
</p>
<? }?>
</td>
          </tr>
...
function sum(){
	...
	$("select[name^=item_qty]").each(function(){
		...
		price=parseFloat( no_comma($(this).next("input[name^=org_price]").val()));
		//fee=parseInt($(this).prev("input").prev("input[name^=item_fee]").val());
...
$(document).ready(function(e) {
	...
	$("input[name^=org_price]").change(function(){
		sum();
	})
