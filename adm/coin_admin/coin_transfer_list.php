...
$sql_common = " from {$g5['cn_transfer_table']} a 
left outer join  {$g5['member_table']} as b on(a.mb_id=b.mb_id) 
left outer join  {$g5['member_table']} as t on(a.tmb_id=t.mb_id) ";
$sql_search = " where (1) ";
...
$sql = " select a.*,b.mb_id,b.mb_hp,b.mb_name,b.mb_recommend,t.mb_hp tmb_hp
...
<option value="a.mb_id"<?php echo get_selected($_GET['sfl'], "a.mb_id"); ?>>보내는 아이디</option>
<option value="b.mb_hp"<?php echo get_selected($_GET['sfl'], "b.mb_hp"); ?>>보내는 연락처</option>

<option value="b.mb_recommend"<?php echo get_selected($_GET['sfl'], "b.mb_recommend"); ?>>보내는 추천인 코드</option>
<option value="a.tmb_id"<?php echo get_selected($_GET['sfl'], "a.tmb_id"); ?>>받는 아이디</option>
<option value="t.mb_hp"<?php echo get_selected($_GET['sfl'], "t.mb_hp"); ?>>받는 이메일</option>
<option value="t.mb_recommend"<?php echo get_selected($_GET['sfl'], "t.mb_recommend"); ?>>받는 추천인 코드</option>
...
<form name="fboardlist" id="fboardlist" action="./coin_transfer_list_update.php" onsubmit="return fboardlist_submit(this);" method="post">
...
<th scope="col"><?php echo subject_sort_link('a.mb_id') ?>보내는아이디</th>
<th scope="col"><?php echo subject_sort_link('b.mb_email') ?>연락처</th>
<th scope="col">받는아이디</th>
<th scope="col"><?php echo subject_sort_link('a.tr_coin') ?>코인</a></th>
        <th scope="col">요청수량</th>
<th scope="col">수수료</th>
        <th scope="col">이체수량</th>
<!--th scope="col">이체계좌</th-->
        <th scope="col">이체처리일</th>
        <th scope="col"><?php echo subject_sort_link('a.tr_stats') ?>이체상태</a></th>
        <th scope="col">관리</th>
    </tr>
    </thead>
    <tbody>
    ...
    for ($i=0; $row=sql_fetch_array($result); $i++) {
        ...
        $one_update = '<a href="./coin_transfer_form.php?w=u&amp;tr_no='.$row['tr_no'].'&amp;'.$qstr.'" class="btn btn_03">관리</a>';
...
<td class='td_center mb-info-open' data-id='<?php echo $row['tmb_id'] ?>' ><?php echo $row['stmb_id']? '<b>'.$row['stmb_id'].'</b> @ ':'' ?><?php echo $row['tmb_id'] ?></td>
<td ><?=$g5['cn_cointype'][$row['tr_token']]?></td>
       <td class="text-right"><?php echo number_format2($row['tr_amt']) ?></td>
<td class="text-right"><?php echo number_format2($row['tr_fee']) ?> <?=$g5['cn_cointype'][$row['tr_fee_token']]?></td>
       <td class="text-right"><?php echo $row['tr_stats']=='3'? number_format2($row['tr_set_tamt'])." ".$g5['cn_cointype'][$row['tr_set_token']]:'-'?></td>
<!--td  class="td_num_c3"><?php echo $row['account'] ?></td-->
        <td class="td_datetime">
		<?php echo $row['tr_stats']=='3'? $row['tr_set_date']:''?>
        </td>
        <td class="td_num_c3"><label for="deposit_status<?php echo $i; ?>" class="sound_only">이체상태</label>
          <span class="local_sch01 local_sch">
          <select id="tr_stats[<?php echo $i ?>]" name="tr_stats[<?php echo $i ?>]" <?=$disabled?> >
            <?
			foreach($g5['cn_instats'] as $k=>$v) echo "<option value='{$k}' ".($row['tr_stats']==$k ? 'selected':'').">{$v}</option>";
			?>
          </select>
          </span></td>
...
<div class="btn_fixed_top">
    ...
    <a href="./coin_transfer_form.php" id="rt_add" class="btn_01 btn">이체추가</a>
</div>
