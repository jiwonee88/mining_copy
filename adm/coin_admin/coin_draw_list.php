...
$sql_common = " from {$g5['cn_draw_table']} a left outer join  {$g5['member_table']} as b on(a.mb_id=b.mb_id) ";
$sql_search = " where (1) ";
...
$sql = " select a.*,b.mb_id,b.mb_email,b.mb_name,b.mb_recommend ,b.mb_hp {$sql_common} {$sql_search} {$sql_order} limit {$from_record}, {$rows} ";
...
<form name="fboardlist" id="fboardlist" action="./coin_draw_list_update.php" onsubmit="return fboardlist_submit(this);" method="post">
...
<th scope="col"><?php echo subject_sort_link('a.in_coin') ?>코인</a></th>
        <th scope="col">출금수량</th>
<th scope="col">수수료</th>
        <th scope="col">출금수량</th>
<th scope="col">출금주소</th>
        <!--th scope="col">출금계좌</th-->
        <th scope="col">출금처리일</th>
        <th scope="col"><?php echo subject_sort_link('a.in_status') ?>출금상태</a></th>
        <th scope="col">관리</th>
    </tr>
    </thead>
    <tbody>
    ...
    for ($i=0; $row=sql_fetch_array($result); $i++) {
        ...
        $one_update = '<a href="./coin_draw_form.php?w=u&amp;dr_no='.$row['dr_no'].'&amp;'.$qstr.'" class="btn btn_03">관리</a>';
    ...
    <tr class="<?php echo $bg; ?>">
        <td class="td_chk">
            ...
            <input type="hidden" name="dr_no[<?php echo $i ?>]" value="<?php echo $row['dr_no'] ?>">
        </td>
       <td  class="td_num">
       <?=$list_num?>
        </td>
...
<td ><?=$g5['cn_cointype'][$row['dr_token']]?></td>
       <td class="text-right"><?php echo number_format2($row['dr_amt']) ?> <?=$g5['cn_cointype'][$row['dr_token']]?></td>
<td class="text-right"><?php echo number_format2($row['dr_fee']) ?>
<?=$g5['cn_cointype'][$row['dr_token']]?></td>
       <td class="text-right"><?php echo number_format2($row['dr_set_tamt']) ?> <?=$g5['cn_cointype'][$row['dr_set_token']]?></td>
<td><?php echo $row['dr_wallet_addr'] ?></td>
       <!--td  class="td_num_c3"><?php echo $row['account'] ?></td-->
        <td class="td_datetime">
		<?php echo $row['dr_stats']=='3'? $row['dr_set_date']:''?>
        </td>
        <td class="td_num_c3"><label for="deposit_status<?php echo $i; ?>" class="sound_only">출금상태</label>
          <span class="local_sch01 local_sch">
          <select id="dr_stats[<?php echo $i ?>]" name="dr_stats[<?php echo $i ?>]" <?=$disabled?> >
            <?
			foreach($g5['cn_instats'] as $k=>$v) echo "<option value='{$k}' ".($row['dr_stats']==$k ? 'selected':'').">{$v}</option>";
			?>
          </select>
          </span></td>
...
<div class="btn_fixed_top">
    ...
    <a href="./coin_draw_form.php" id="rt_add" class="btn_01 btn">출금추가</a>
</div>
