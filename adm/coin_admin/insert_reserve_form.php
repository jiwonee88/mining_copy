...
if ($w == '') {
    ...
    $data= sql_fetch("select * from {$g5['cn_reserve_table']} where in_no='$in_no'");	
         ...
         <colgroup>
          ...
          <tr>
            ...
            <select id="in_stats" name="in_stats"  >
              <?
			foreach($g5['cn_instats'] as $k=>$v) echo "<option value='{$k}' ".($data['in_stats']==$k ? 'selected':'').">{$v}</option>";
			?>
            </select>
...
<tr>
<th scope="row"><label for="in_set_amt2">서브아이디</label></th>
<td>
<?
if(!$data['in_no']){?>
<input name="smb_id" type="text"  class="frm_input " id="smb_id"  value="" size="20" <?=$disabled?>/>
<? }else{			

echo $data['smb_id']?$data['smb_id']:'-';
...
<input type="hidden" name="smb_id" value="<?php echo $data[smb_id]?>">
        ...
        <tr>
          ...
          <td><span class="local_sch01 local_sch">
            <select name="in_token" class="form-control input-sm  w-auto  mb-1" id="in_token">
...
<td><input name="in_rsv_amt" type="text"  class="required frm_input number-comma" id="in_rsv_amt" required="required" value="<?php echo number_format2($data['in_rsv_amt'])?>" size="20" <?=$disabled?>/></td>
        ...
        <tr>
          <th scope="row"><label for="in_wallet_addr">입금주소</label></th>
          <td>	
