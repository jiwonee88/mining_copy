...
sum(if(cn_item='e',ct_buy_price,0)) e_buy_price,
...
sum(if(cn_item='e',ct_sell_price,0)) e_sell_price,
...
sum(ct_sell_price) ct_sell_price 
...
left outer join  (select mb_id cmb_id, count(*) ct_cnt, sum(ct_sell_price) ct_sell_price,  sum(ct_buy_price) ct_buy_price from  {$g5['cn_item_cart']}  where is_soled!='1' group by mb_id)  as c on(c.cmb_id=a.mb_id) ");

$tot2=array();
$tot2_cnt=$tot2_amt=0;
$re=sql_query("select cn_item, count(code) cnt, sum(ct_sell_price) ct_sell_price,  sum(ct_buy_price) ct_buy_price from {$g5['cn_item_cart']} where is_soled!='1' group by cn_item",1);
