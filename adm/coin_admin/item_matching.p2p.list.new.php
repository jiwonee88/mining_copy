...
<input type="submit" name="act_button" value="매칭결과승인처리" onclick="document.pressed=this.value" class="btn_01 btn">
<br><br>
...
<form name="fboardlist" id="fboardlist" action="./item_matching.p2p.list.php" onsubmit="return fboardlist_submit(this);" method="post">
...
<tr>
<th scope="col"><?php echo subject_sort_link('c.ct_class') ?></th>
...
<td ><br><?php echo $g5['cn_item'][$row['ccn_item']][name_kr] ?></td>
...
<td ><span class="td_datetime"><?php echo str_replace(" ","<br>",$row['ct_wdate'])?></span></td>
<td ><span class="td_datetime"><?php echo $row['ct_validdate']?></span></td>
<td ><span class="td_datetime"><?php echo $row['ct_days']?><br>
...
<a href="./item_matching.p2p.list.excel.php?<?=$qstr?>" class='btn btn_02'>매칭내역엑셀출력</a>    
</div>
...
{	
	...
	if(!confirm("현재 내역을 승인 처리 합니까? 매칭 상품은 즉시 지급됩니다")) {
		return false;
	}else return true;
