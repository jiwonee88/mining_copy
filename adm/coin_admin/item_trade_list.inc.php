...
b.mb_id,b.mb_email,b.mb_name,b.mb_trade_get_claim,b.mb_trade_put_claim,b.mb_trade_penalty,b.mb_trade_penalty_date {$sql_common} {$sql_search} {$sql_order} limit {$from_record}, {$rows} ";
...
<option value="buy_recommend"  <?php echo get_selected($_GET['sfl'], "buy_recommend"); ?>>구매자 추천인 아이디</option>
<option value="sell_recommend"  <?php echo get_selected($_GET['sfl'], "sell_recommend"); ?>>판매자 추천인 아이디</option>
</select>
...
<input type="hidden" name="from_menu" value="<?php echo $submenu?>">
...
<th scope="col">패널티</th>
    ...
    <tr class="<?php echo $bg; ?>">
        <td rowspan="2" class="td_chk">
            ...
            <input type="checkbox" name="chk[]" value="<?php echo $i ?>" id="chk_<?php echo $i ?>" <?=$disabled?> <?=$row['tr_stats']=='3'?'disabled':''?> >
...
<td class="td_date2"><?php echo str_replace(" ","<br>", $row['tr_rdate'])?></td>
        <td class="td_num_c3"><label for="deposit_status<?php echo $i; ?>" class="sound_only">상태</label>
          <select id="tr_stats[<?php echo $i ?>]" name="tr_stats[<?php echo $i ?>]" <?=$row['tr_stats']=='3'?'disabled':''?> >
            <?
...
<tr class="<?php echo $bg; ?>">
<td class='mb-info-open' data-id='<?php echo $row['fmb_id'] ?>'><?php echo $row['fsmb_id'] ?> @<?php echo $row['fmb_id'] ?></td>
