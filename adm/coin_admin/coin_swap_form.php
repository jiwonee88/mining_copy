    ...
    <div class="tbl_frm01 tbl_wrap">
        <table>
         <colgroup>
          ...
          <tr>
            <th scope="row"><label for="sw_stats">스왑상태</label></th>
            <td>           
            <select id="sw_stats" name="sw_stats"  >
              <?
			foreach($g5['cn_instats'] as $k=>$v) echo "<option value='{$k}' ".($data['sw_stats']==$k ? 'selected':'').">{$v}</option>";
			?>
            </select>
        ...
        <tbody>
        <tr>
            <th scope="row"><label for="tmb_email">회원</label></th>
            <td>
            <?
			if(!$data['sw_no']){?>
