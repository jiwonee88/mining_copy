...
<td class='mb-info-open' data-id='<?php echo $row['mb_id'] ?>' ><?php echo $row['smb_id'] ?>@<?php echo $row['mb_id'] ?><i class="far fa-question-circle"></i></td>
...
<td class="td_num_c3"><label for="deposit_status<?php echo $i; ?>" class="sound_only">입금상태</label>
  ...
  <select id="it_stats[<?php echo $i ?>]" name="it_stats[<?php echo $i ?>]" <?=$disabled?> >
    <?
			foreach($g5['purchase_stat'] as $k=>$v) echo "<option value='{$k}' ".($row['it_stats']==$k ? 'selected':'').">{$v}</option>";
			?>
  </select>
  </span></td>
