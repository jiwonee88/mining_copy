<?
if($sub_menu == "700615" || $sub_menu == "700766"){
	$mb_search = " and a.mb_14='vip' ";	
	
}else if($sub_menu == "700610"  || $sub_menu == "700765"){
	$mb_search = " and a.mb_14!='vip' ";	
...
left outer join  (select mb_id cmb_id,count(*) ct_cnt,sum(ct_buy_price) ct_buy_price  from {$g5['cn_item_cart']} where  is_soled != '1' group by mb_id)  as c on(c.cmb_id=a.mb_id) 
...
left outer join  (select mb_id fmb_id, count(*) ftr_cnt, sum(ct_buy_price) ftr_price from $matching_table where  tr_wdate='$date_stx'  group by mb_id)  as f on(f.fmb_id=a.mb_id)
where (1) $mb_search 
",1);
...
<th rowspan="2" scope="col" >활성계정</a></th>
		...
		<th rowspan="2" scope="col" >총매도</th>
		<th rowspan="2" scope="col" >총매수</th>
...
<td class="td_datetime"><?php echo number_format($tot[ftr_cnt]); ?></td>	 
<td class="td_datetime"><?php echo number_format($tot[tr_cnt]); ?></td>
<td class="td_datetime"><?php echo number_format($tot[all_auto_cnt]); ?></td>
<td class="td_datetime"><?php echo round($tot[tr_cnt]/$tot[all_auto_cnt]*100,2) ?>%</td>
...
<th rowspan="2" id="mb_list_cert" scope="col">설정금액</th>
<th rowspan="2" id="mb_list_cert" scope="col">보유금액</th>
<th rowspan="2" id="mb_list_cert" scope="col">거래(구매중)</th>
<th rowspan="2" scope="col" ><?php echo subject_sort_link('sc_cnt',"&date_stx=$date_stx") ?>총계정</a></th>
<th rowspan="2" scope="col" ><?php echo subject_sort_link('active_cnt',"&date_stx=$date_stx") ?>활성계정</a></th>
    ...
    <?
		...
		?>
        <th colspan="3"  scope="col"><?php echo subject_sort_link('ac_auto_'.$k) ?><?=$v[name_kr]?></th>
...
<th rowspan="2" scope="col" ><?php echo subject_sort_link('m_ratio',"&date_stx=$date_stx", 'desc') ?>매칭률</th>
		...
		//계정별
		foreach($g5['cn_cointype'] as $k=> $v){ ?>
        <th rowspan="2" id="mb_list_id" scope="col"><?php echo subject_sort_link('ac_point_'.$k) ?>총<?=$v?>수량</th>
        <? }?>
...
<td><?=number_format2($row['tr_price_org'])?></td>
<td class='td_center'><?=$row['sc_cnt']?></td>
<td class='td_center'><?=$row['active_cnt']?></td>
		
      <td class='td_center'><?=round($row['open_r']*100,1)?>%</td>
...
<td><?php echo $row[all_auto_cnt]; ?></td>
<td><?php echo round($row[m_ratio]*100,2) ?>%</td>
