...
if ($w == '') {
	...
	$data['sk_sdate1']=date("Y-m-d");
	$data['sk_sdate2']=date("H:i:s");
...
?>
<form name="fcommonform" id="fcommonform" action="./staking_update.php" onsubmit="return fcommonform_submit(this)" method="post" enctype="multipart/form-data">
<input type="hidden" name="w" value="<?php echo $w ?>">
<input type="hidden" name="sk_no" value="<?php echo $sk_no ?>">
    ...
    <div class="tbl_frm01 tbl_wrap">
        <table>
         <colgroup>
          ...
          <tr>
            <th scope="row"><label for="sk_stats">상태</label></th>
            <td>
            <?=help('미입금 상태인경우 주기적으로 입급여부 검사후 자동으로 처리됩니다')?>
            <select id="sk_stats" name="sk_stats"  >
              <?
			foreach($g5['stake_stat'] as $k=>$v) echo "<option value='{$k}' ".($data['sk_stats']==$k ? 'selected':'').">{$v}</option>";
			?>
            </select>
        ...
        <tr>
          <th scope="row"><label for="rt_name">코인</label></th>
          <td><span class="local_sch01 local_sch">
            <select name='sk_token' class="form-control input-sm  w-auto  mb-1" id="sk_token" <?=$readonly?> <?=$disabled?> >
              <?
foreach($g5['cn_cointype'] as $k=>$v){	
	?>
              <option value='<?=$k?>' <?=$data['sk_token']==$k?'selected':''?> >
                <?=$v?>
                </option>
              <? }?>
            </select>
...
<tr>
<th scope="row"><label for="in_set_amt2">수량</label></th>
<td><input name="sk_amt" type="text"  class="required frm_input number-comma" id="sk_amt" required="required" value="<?php echo number_format2($data['sk_amt'])?>" size="20" <?=$disabled?>/></td>
</tr>
         
		 <?
		if($data['sk_no']){?>
          <tr>
            <th scope="row"><label for="sk_sdate2">기간</label></th>
            <td><?=$data['sk_stats']=='2'?$data['sk_sdate']:'-'?> ~ <?=$data['sk_stats']=='3'?$data['sk_edate']:($data['sk_stats']=='2'?'진행중':'')?></td>
          </tr>
...
<div class="btn_fixed_top">
	<a href="./staking_list.php?<?=$qstr?>"  class=" btn_02 btn">전체목록</a>
