...
if($coin_stx) {
	$sql_search .= " and a.in_set_token = '$coin_stx' ";
...
$g5['title'] = ($g5[cn_cointype]['i']).'구매내역';
...
<option value="a.in_txn_id"<?php echo get_selected($_GET['sfl'], "a.in_txn_id"); ?>>입금자/txid</option>
...
<th rowspan="2" scope="col"><?php echo subject_sort_link('a.mb_id') ?>아이디</th>
        <th rowspan="2" scope="col"><?php echo subject_sort_link('b.mb_email') ?>구매상품</a></th>
...
<th rowspan="2" scope="col">입금자 / TXID</th>
<th rowspan="2" scope="col">신청일</th>
        <!--th scope="col">입금계좌</th-->
    <th rowspan="2" scope="col">최종처리</th>
<th colspan="3" scope="col">지갑 참조 결과</th>
        <th rowspan="2" scope="col"><?php echo subject_sort_link('a.in_status') ?>입금상태</a></th>
        <th rowspan="2" scope="col">관리</th>
    </tr>
<tr>
<th scope="col">접수시 잔액</th>
...
<td ><?php echo $row['in_item'] ?> (<strong>×<?php echo $row['in_item_qty'] ?></strong>)</td>
<td ><?=$g5['cn_cointype'][$row['in_token']]?></td>
       ...
       <td class="text-right <?=$row['in_set_token']=='b'?'fred':'fblue';?>"><?php echo number_format2($row['in_set_amt']) ?> <?=$g5['cn_cointype'][$row['in_set_token']]?></td>
<td class="td_datetime"><?php echo $row['in_txn_id']?></td>
<td class="td_datetime"><?php echo substr($row['in_wdate'],0,16)?></td>
