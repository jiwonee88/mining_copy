...
while($sdata=sql_fetch_array($re)){
	...
	foreach($multidata as $data){				
		...
		if( $w!='s'){
			...
			//회원구분 여부
			if($is_vip =='y')  $vip_search2 = " and  b.mb_14='vip' ";	
			else $vip_search2 = " and  b.mb_14!='vip' ";		
		
		    	
	
			$sql_search = " where   a.mb_id!='$sdata[mb_id]' and mb_10 not in ('1','2')  $vip_search2 $target_buyer_search  $except_buyer_search";
			...
			$lines.=" / 설정금액 : <strong>".number_format2($row[mb_trade_amtlmt]-$row[enable_paid]) ."</strong> = ".  number_format2($row[mb_trade_amtlmt])." - ". number_format2($row[enable_paid])." /  상품금액 :  <strong>".number_format2($data[item_price])."</strong>";
