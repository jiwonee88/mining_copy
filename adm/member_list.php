...
left outer join (select mb_id cmb_id, sum(ct_buy_price) ct_buy_price from `coin_item_cart` where  is_soled!='1' group by mb_id) as b on (b.cmb_id=a.mb_id ) 
";
...
//관리자
if($sub_menu == "600200"){
	$sql_search .= " and (a.mb_level='10' or a.mb_level='9')  and a.mb_10 not in ('1','2')  ";
	$return_page="member_list_adm.php";


//회원 회원
}else if($sub_menu == "200120"){
	$sql_search .= " and (a.mb_level between 2 and 8 )  and a.mb_10 not in ('1','2') ";
	$return_page="member_list_partner.php";

//임시회원
}else if($sub_menu == "200150"){
	$sql_search .= " and (a.mb_level='1')  and a.mb_10 not in ('1','2') ";
	$return_page="member_list_ready.php";
	
	
//vip 회원
}else if($sub_menu == "200190"){
	$sql_search .= " and (a.mb_14='vip' ) and a.mb_10 not in ('1','2') ";
	$return_page="member_list_vip.php";

//탈퇴/삭제 회원
}else if($sub_menu == "200130"){
	 $sql_search .= " and a.mb_10 in ('1','2') ";
...
if ($_REQUEST['date_start_stx']) {	
    $sql_search .= " and a.mb_leave_date >= '".str_replace("-","",$date_start_stx)."'";	
	$qstr.="&date_start_stx=$date_start_stx";		
}
if ($_REQUEST['date_end_stx']) {
    $sql_search .= " and a.mb_leave_date <= '".str_replace("-","",$date_end_stx)."'";	
	$qstr.="&date_end_stx=$date_end_stx";
}
if ($mb9_stx != '')
    $sql_search .= " and a.mb_9 = '{$mb9_stx}' ";
...
// 탈퇴회원수
$sql = " select count(a.*) as cnt {$sql_common} {$sql_search} and a.mb_leave_date <> '' {$sql_order} ";
$row = sql_fetch($sql);
$leave_count = $row['cnt'];

// 차단회원수
$sql = " select count(a.*) as cnt {$sql_common} {$sql_search} and a.mb_intercept_date <> '' {$sql_order} ";
$row = sql_fetch($sql);
...
else if($sub_menu == "200190") $g5['title'] .="-VIP회원";
...
$sql = " select *,b.ct_buy_price  {$sql_common} {$sql_search} {$sql_order} limit {$from_record}, {$rows} ";
...
<select name="sfl" id="sfl">
	...
	<option value="mb_12"<?php echo get_selected($_GET['sfl'], "mb_12"); ?>>UID</option>
...
<th id="mb_list_id" scope="col"><?php echo subject_sort_link('mb_recommend') ?>UID</th>
...
<th id="mb_list_id" scope="col">
 서브계정</th>
        ...
        <? }?>
        <!--th scope="col" id="mb_list_cert">
	지갑</th-->
        <th scope="col" id="mb_list_cert">연락처</th>
        <th id="mb_list_lastcall" scope="col"><?php echo subject_sort_link('mb_today_login', '', 'desc') ?>최종접속</a></th>
<th scope="col" ><?php echo subject_sort_link('mb_13', '', 'desc') ?>서비스동의</th>
        <th scope="col" ><?php echo subject_sort_link('mb_datetime', '', 'desc') ?>가입일</th>
        <th scope="col" id="mb_list_mng">관리</th>
    </tr>
    </thead>
...
<td  ><?php echo $row[mb_name] ?></td>
<td  ><?=number_format($row['ct_buy_price'])?></td>
<td  ><?=number_format($row['mb_trade_amtlmt'])?></td>
		
    <td><strong>
	<?php echo $row['mb_14']=='vip'?"[<strong style='color:red'>VIP</strong>]<br>":'' ?>
	<?php echo $g5['member_level_name'][$row['mb_level']] ?></strong></td>
<td  ><?php echo $row['mb_recommend']?$row['mb_recommend']:'-'?></td>
<td  ><?php echo $row['mb_12']?$row['mb_12']:'-'?></td>
      <td  >
	  <?
        if($count['cnt'] > 0 ){?>
	...
	<p><?=$g5['cn_cointype'][$v]?> : <?php echo $row['mb_wallet_addr_'.$v]?$row['mb_wallet_addr_'.$v]:'-'?></p>
     <? }?> 
      ...
      <td><?=$row['mb_hp']?></td>
      <td class="td_date"><?php echo substr($row['mb_today_login'],2,8); ?></td>
<td class="td_date"><?php echo substr($row['mb_13'],0,10); ?></td>
      <td class="td_date"><?php echo substr($row['mb_datetime'],2,8); ?></td>
      <td headers="mb_list_mng" class="td_mng td_mng_s"><?php echo $s_mod ?><?php echo $s_grp ?></td>
    </tr>
    <?php
...
function fmemberlist_submit(f)
{
    if (!is_checked("chk[]")) {
	 ...
	 if(document.pressed == "VIP선정") {
        if(!confirm("선택한 회원을 VIP 회원으로 선정 하시겠습니까?")) {
            return false;
        }
    }
